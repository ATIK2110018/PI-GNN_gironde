...
def roe_flux_2d(h_L, h_R, u_L, u_R, v_L, v_R, h_still_L, h_still_R, nx, ny, g=9.81, h_small=1e-3):
    """
    Well-Balanced Roe/Rusanov Riemann Solver for the 2D Shallow Water Equations.
    Inlined here to remove the dependency on the deleted riemann_solver.py module.
    """
    xi_L = h_L - h_still_L
    xi_R = h_R - h_still_R
    hu_L, hv_L = h_L * u_L, h_L * v_L
    hu_R, hv_R = h_R * u_R, h_R * v_R

    sqrt_hL = torch.sqrt(torch.clamp(h_L, min=h_small))
    sqrt_hR = torch.sqrt(torch.clamp(h_R, min=h_small))
    ...
    wave_speed = torch.clamp(torch.abs(un_Roe) + c_Roe, min=1e-8)

    F_mass  = 0.5 * (F_mass_L  + F_mass_R)  - 0.5 * wave_speed * (xi_R  - xi_L)
    F_mom_x = 0.5 * (F_mom_x_L + F_mom_x_R) - 0.5 * wave_speed * (hu_R  - hu_L)
    F_mom_y = 0.5 * (F_mom_y_L + F_mom_y_R) - 0.5 * wave_speed * (hv_R  - hv_L)
